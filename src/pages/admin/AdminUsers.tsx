  ...
  return (
    <>
                  ...
                  <tr>
                    {(isLearnersMode
                      ...
                      (h, i) => (
                        <th
                          key={i}
                          className={`px-lg py-md font-label-md text-[#1D4532] font-semibold uppercase tracking-wider ${i === 4 ? 'text-right' : ''
                            }`}
                        >
                          {h}
                        </th>
                      ...
                      >
                        ...
                        <td className="px-lg py-md">
                          <div className="flex items-center gap-md">
                            <div className={`relative w-10 h-10 shrink-0 overflow-hidden rounded-full flex items-center justify-center font-bold text-sm ${getAvatarStyle(user.role)}`}>
                              {user.initials}
                              {user.avatar && (
                                <img
                                  src={user.avatar}
                        ...
                        <td className="px-lg py-md">
                          {isLearnersMode ? (
                            ...
                            <span
                              className={`px-sm py-1 text-[12px] font-bold rounded-lg ${roleBadge[user.role] ?? ''
                                }`}
                            >
                              {user.role}
                            </span>
      ...
      {selectedUser && (
        ...
        >
          <div
            className="fixed top-0 right-0 h-full w-[100%] sm:w-[65%] md:w-[55%] lg:w-[45%] bg-white border-l border-[#d1e4fb] shadow-2xl z-[1000] overflow-hidden flex flex-col animate-[slideIn_0.3s_ease-out]"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Modal Header */}
            <div className="px-xl py-lg border-b border-[#d1e4fb] flex justify-between items-center bg-[#EDF7F2]">
              <div className="flex items-center gap-md">
                <div className={`relative w-16 h-16 shrink-0 overflow-hidden rounded-full flex items-center justify-center font-bold text-xl ${getAvatarStyle(selectedUser.role)}`}>
            ...
            <div className="flex-1 overflow-y-auto p-xl space-y-6 bg-white custom-scrollbar">
              ...
              {/* Account information */}
              <section className="rounded-xl border border-[#d1e4fb] bg-white p-lg">
                <h4 className="mb-md text-label-md font-bold uppercase tracking-wider text-[#1D4532]">
                  Thông tin tài khoản
                </h4>
                <div className="grid grid-cols-1 gap-md sm:grid-cols-2">
                  <div>
                    <p className="text-[12px] text-[#5e5e5b]">Email đăng nhập</p>
                    <p className="mt-1 break-all text-[14px] font-medium text-on-surface">{selectedUser.email || 'Chưa cập nhật'}</p>
                  </div>
                  <div>
                    <p className="text-[12px] text-[#5e5e5b]">Vai trò trên hệ thống</p>
                    <p className="mt-1 text-[14px] font-medium text-on-surface">{selectedUser.role}</p>
                  </div>
                  <div>
                    <p className="text-[12px] text-[#5e5e5b]">Ngày đăng ký</p>
                    <p className="mt-1 text-[14px] font-medium text-on-surface">{selectedUser.registeredAt || 'Chưa cập nhật'}</p>
                  </div>
                  {selectedUser.role !== 'Người học' && selectedUser.specialty && (
                    <div>
                      <p className="text-[12px] text-[#5e5e5b]">Chuyên môn nhạc cụ</p>
                      <p className="mt-1 text-[14px] font-medium text-on-surface">{selectedUser.specialty}</p>
                    </div>
                  )}
                </div>
              </section>
              ...
              <div className="grid grid-cols-3 gap-md">
                {[
                  ...
                  <div
                    key={i}
                    className="p-md bg-white rounded-lg border border-[#d1e4fb]"
                  >
                    ...
                    {selectedUser.activities.map((act, i) => (
                      <div key={i} className="flex gap-md">
                        <div className="flex flex-col items-center">
                          <div
                            className={`w-2 h-2 rounded-full mt-2 ${i === 0 ? 'bg-[#cca730]' : 'bg-outline-variant'
                              }`}
                          />
                  ...
                  <section>
                    ...
                    <div className="flex flex-wrap gap-sm">
                      {selectedUser.instruments.map((inst) => (
                        <span
                          key={inst}
                          className="px-md py-sm bg-[#EDF7F2] border border-[#d1e4fb] rounded-full text-label-md text-[#1D4532] flex items-center gap-xs"
                        >
        ...
        <AnimatePresence>
          {isAddDrawerOpen && (
            <>
              ...
              {/* Slide-in Drawer */}
              <motion.div
                className="fixed top-0 right-0 h-full w-[100%] sm:w-[65%] md:w-[55%] lg:w-[45%] bg-white border-l border-[#d1e4fb] shadow-2xl z-[1000] overflow-hidden flex flex-col"
                initial={{ x: '100%' }}
                animate={{ x: 0 }}
                exit={{ x: '100%' }}
              ...
              >
                {/* Drawer Header */}
              <div className="px-lg py-md border-b border-[#d1e4fb] flex justify-between items-center bg-[#EDF7F2]">
                  <div>
                ...
                <form onSubmit={submitAddUser} className="flex-1 min-h-0 flex flex-col">
                  <div className="flex-1 min-h-0 overflow-y-auto p-lg custom-scrollbar">
                    <div className="bg-white border border-[#d1e4fb] rounded-2xl p-md shadow-sm space-y-sm">
                      {/* Name Input */}
                  ...
                  <div className="shrink-0 px-lg py-md border-t border-[#d1e4fb] bg-white flex gap-md">
                    <button
                      type="button"
                      onClick={() => setIsAddDrawerOpen(false)}
                      className="flex-1 flex items-center justify-center gap-sm bg-[#e1dfdb] text-on-surface py-md rounded-lg font-bold hover:bg-[#c8c6c2] active:scale-[0.98] transition-all border border-outline-variant/30"
                    >
                      <X className="w-5 h-5" />
                      Hủy bỏ
                    </button>
          ...
          {isEditDrawerOpen && editingUser && (
            <>
              ...
              <div className="px-lg py-md border-b border-[#d1e4fb] flex justify-between items-center bg-[#EDF7F2]">
                ...
                {/* Drawer Body */}
                <form onSubmit={submitEditUser} className="flex-1 min-h-0 flex flex-col">
                  <div className="flex-1 min-h-0 overflow-y-auto p-lg custom-scrollbar">
                    <div className="bg-white border border-[#d1e4fb] rounded-2xl p-lg shadow-sm space-y-md min-h-full flex flex-col justify-between">
                      {/* ID Field (Read-only) */}
                      <div className="flex flex-col gap-xs">
                        <label className="font-label-sm text-on-surface-variant font-semibold uppercase tracking-wider text-xs">
                          Mã thành viên (ID)
                        ...
                        <input
                          ...
                          className="w-full bg-[#f1f3f2] border border-[#d1e4fb] rounded-lg p-md text-body-md text-on-surface-variant/80 select-none outline-none"
                        />
                      </div>
                  ...
                  <div className="shrink-0 px-lg py-md border-t border-[#d1e4fb] bg-white flex gap-md">
                    ...
                    <button
                      ...
                      className={`flex-1 flex items-center justify-center gap-sm py-md rounded-lg font-bold active:scale-[0.98] transition-all shadow-md ${isEditFormValid
                          ? 'bg-[#1D4532] text-white hover:bg-[#1D4532]/95 cursor-pointer'
