...
import {
  CalendarDays, Camera, Check, ChevronLeft,
...
const ProfilePage = ({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#f7f9ff] via-[#edf4ff] to-[#f0f7f0] flex flex-col">
      {/* ── Top bar ── */}
      <header className="h-14 flex items-center px-6 bg-white/80 backdrop-blur-md border-b border-[#d1e4fb] sticky top-0 z-20">
        <button
          onClick={() => navigate(backTo)}
          className="flex items-center gap-2 text-sm font-medium text-[#5e5e5b] hover:text-primary transition-colors group"
        >
          <ChevronLeft className="w-4 h-4 group-hover:-translate-x-0.5 transition-transform" />
          Quay lại Dashboard
        </button>
        <span className="ml-auto text-sm font-semibold text-primary">Hồ sơ cá nhân</span>
      </header>
                ...
                {newPassword.length > 0 && (
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-[#5e5e5b]">
                      {newPassword.length < 6 ? 'Quá ngắn' : newPassword.length < 9 ? 'Trung bình' : newPassword.length < 12 ? 'Mạnh' : 'Rất mạnh'}
                    </span>
                  </div>
                )}
